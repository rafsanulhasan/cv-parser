    ...
    <div style="padding: 20px; font-family: sans-serif; max-width: 800px; margin: 0 auto;">
        ...
        <div *ngIf="showSettings" style="padding: 15px; background: #fff; border-top: 1px solid #ddd;">
          ...
          <div>
            <div style="margin-bottom: 15px;">
              ...
              <select [ngModel]="selectedEmbeddingModelId" (ngModelChange)="onEmbeddingModelChange($event)" style="width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ccc;">
            ...
            <div style="margin-bottom: 15px;">
              ...
              <select [ngModel]="selectedChatModelId" (ngModelChange)="onChatModelChange($event)" style="width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ccc;">
          ...
          <!-- Pull Progress -->
          <div *ngIf="isPullingModel" style="margin-top: 15px; padding: 10px; background: #e2e3e5; border-radius: 4px;">
            ...
            <div style="height: 10px; background: #fff; border-radius: 5px; overflow: hidden;">
                <div [style.width.%]="pullProgress.percent" style="height: 100%; background: #007bff; transition: width 0.3s ease;"></div>
            </div>
            <p style="margin: 5px 0 0; font-size: 0.8em; color: #666;">{{ pullProgress.status }}</p>
          </div>
      ...
      <div style="margin-bottom: 20px; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background: #f5f5f5;">
        ...
        <div style="display: flex; align-items: center;">
            <input type="file" (change)="onFileSelected($event)" accept=".txt,.md,.json,.pdf,.docx,.doc" [disabled]="isProcessing || isPullingModel">
...
export class AppComponent implements OnInit {
  ...
  pullProgress = { status: '', completed: 0, total: 0, percent: 0 };
  ...
  onEmbeddingModelChange ( modelId: string ) {
    // Don't set it yet, check status first
  ...
  checkOllamaModelStatus ( modelId: string, type: 'chat' | 'embedding' ) {
    if ( this.selectedProvider === 'ollama' ) {
      // Check both chat and embedding models
      ...
      if ( model ) {
        if ( model.isInstalled === false ) {
          // Prompt to install
          if ( confirm( `Model '${ model.name }' is not installed. Do you want to pull it now? (This may take a while)` ) ) {
            this.pullOllamaModel( modelId, type );
          } else {
            alert( 'Model selection cancelled. Please select an installed model to proceed.' );
            this.refreshLibrary(); // Brute force UI reset
          }
        } else {
          // It is installed, safe to set
          if ( type === 'chat' ) {
  ...
  async pullOllamaModel ( modelName: string, type: 'chat' | 'embedding' ) {
    ...
    this.pullProgress = { status: 'Starting...', completed: 0, total: 0, percent: 0 };

    try {
      await this.ollamaService.pullModel( modelName, ( status, completed, total ) => {
        const percent = total > 0 ? Math.round( ( completed / total ) * 100 ) : 0;
        this.pullProgress = { status, completed, total, percent };
      } );
